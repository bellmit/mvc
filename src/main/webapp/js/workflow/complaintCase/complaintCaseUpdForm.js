var complaintCaseUpdForm = {
	...
	init:function(caseCodeStr){
		complaintCaseUpdForm.tableHtml = $('#complaintArea').html();
		complaintCaseUpdForm.caseCodeStr = caseCodeStr;
		complaintCaseUpdForm.numberMap = new TabMap();
		complaintCaseUpdForm.relationComplaintMap = new TabMap();
		var commonParamForm = $('#commonParamForm');
		$.post(commonParamForm.attr("action"), {}, function(data){
			complaintCaseUpdForm.complaintObjTypeList = data.complaintObjTypeList;
			complaintCaseUpdForm.complaintGistList = data.complaintGistList;
			complaintCaseUpdForm.schedulingTypeList = data.schedulingTypeList;
			complaintCaseUpdForm.initComplaintObj();
			// 关闭滚动条
			base.closepageloading();
		});
		
		complaintCaseUpdForm.initRelationComplaint(caseCodeStr);
		//机构列表初始化
		$('#orgDialogBtn').bind('click',function(){
			complaintCaseUpdForm.orgDialogBtnClick();
		});
		//员工列表初始化
		$('#operDialogBtn').bind('click',function(){
			complaintCaseUpdForm.operDialogBtnClick();
		});
		//供应商列表初始化
		$('#supplierDialogBtn').bind('click',function(){
			complaintCaseUpdForm.supplierDialogBtnClick();
		});
		//车牌号
			...
			if($('#policyLicId').val() != ""){
				...
				$.post(relationComplaintForm.attr("action"), relationComplaintForm.serialize(), function(data){
					if(data==null || data.length <= 0){
						...
						complaintCaseUpdForm.relationComplaintMap.remove('policyLicId');
					} else {
						complaintCaseUpdForm.relationComplaintMap.remove('policyLicId');
						complaintCaseUpdForm.relationComplaintMap.put('policyLicId',data);
					}
					complaintCaseUpdForm.setRelationComplaint();
					// 关闭滚动条
					base.closepageloading();
				});
			} else {
			...
			if($('#cplPhone1').val() != "" && $('#cplPhone2').val() != "" && $('#cplPhone3').val() != ""){
				...
				$.post(relationComplaintForm.attr("action"), relationComplaintForm.serialize(), function(data){
					if(data==null || data.length <= 0){
						...
						complaintCaseUpdForm.relationComplaintMap.remove('cplPhone');
					} else {
						complaintCaseUpdForm.relationComplaintMap.remove('cplPhone');
						complaintCaseUpdForm.relationComplaintMap.put('cplPhone',data);
					}
					complaintCaseUpdForm.setRelationComplaint();
					// 关闭滚动条
					base.closepageloading();
			...
			if($('#cplMobile').val() != ""){
				...
				$.post(relationComplaintForm.attr("action"), relationComplaintForm.serialize(), function(data){
					if(data==null || data.length <= 0){
						...
						complaintCaseUpdForm.relationComplaintMap.remove('cplMobile');
					} else {
						complaintCaseUpdForm.relationComplaintMap.remove('cplMobile');
						complaintCaseUpdForm.relationComplaintMap.put('cplMobile',data);
					}
					complaintCaseUpdForm.setRelationComplaint();
					// 关闭滚动条
					base.closepageloading();
	...
	initRelationComplaint:function(caseCodeStr){
		if(caseCodeStr != null && caseCodeStr != ""){
			...
			if(policyLicId.length > 0){
				complaintCaseUpdForm.relationComplaintMap.put('policyLicId',policyLicId);
			}
			if(cplPhone.length > 0){
				complaintCaseUpdForm.relationComplaintMap.put('cplPhone',cplPhone);
			}
			if(cplMobile.length > 0){
				complaintCaseUpdForm.relationComplaintMap.put('cplMobile',cplMobile);
			}
			complaintCaseUpdForm.setRelationComplaint();
		}
	},
	...
	setRelationComplaint:function(){
		...
		var noEqCaseCode = [];
//		$.each(complaintCaseUpdForm.relationComplaintMap.keys(),function(i,key){
//			var caseCodeList = complaintCaseUpdForm.relationComplaintMap.get(key);
...
//		});
	...
	initComplaintObj:function(){
		...
		$.post(complaintSubObjectListForm.attr("action"), complaintSubObjectListForm.serialize(), function(data){
			if(data != null && data.length > 0){
				$.each(data,function(i,complaintSubObj){
					complaintCaseUpdForm.createComplaintArea(complaintCaseUpdForm.number,complaintSubObj);
				});
			} else {
				complaintCaseUpdForm.createComplaintArea(complaintCaseUpdForm.number);
			}
	...
	createComplaintArea:function(index,complaintSubObj){
		complaintCaseUpdForm.numberMap.put(index,index);
		var table = $('#complaintArea');
		
		var tr1 = complaintCaseUpdForm.createTR(table,complaintCaseUpdForm.complaintTr1+index);
		complaintCaseUpdForm.createTitle('style="width:13%"','<span class="txt-impt">*</span>&nbsp;被投诉对象类型：',tr1);
		var detail = complaintCaseUpdForm.createDetail('','',tr1);
		var complaintObjTitle = complaintCaseUpdForm.createTitle('','',tr1);
		var complaintObjDetail = complaintCaseUpdForm.createDetail('','',tr1);
		complaintObjDetail.attr('colspan','2');
		complaintCaseUpdForm.createComplaintObjType('complaintObjType'+index,detail,complaintObjTitle,complaintObjDetail,index,complaintSubObj);
		
		var tr2 = complaintCaseUpdForm.createTR(table,complaintCaseUpdForm.complaintTr2+index);
		complaintCaseUpdForm.createTitle('','<span class="txt-impt">*</span>&nbsp;投诉要点：',tr2);
		detail = complaintCaseUpdForm.createDetail('style="border-right:none;"','',tr2);
		complaintCaseUpdForm.createComplaintGist('complaintGist'+index,detail,index);
		var multiSelectLeft = '<select id="multiSelectLeft'+index+'" style="width:220px;height:100px;" multiple></select>';
		complaintCaseUpdForm.createDetail('style="border-right:none;border-left:none;"',multiSelectLeft,tr2);
		detail = complaintCaseUpdForm.createDetail('style="text-align:center;border-right:none;border-left:none;"','',tr2);
		//******************选择按钮******************//
		...
		btn.bind('click', function(){
			...
			complaintCaseUpdForm.setMultiSelectRight(index);
		});
		//******************删除按钮******************//
		$('<br>').appendTo(detail);
		...
		detail = complaintCaseUpdForm.createDetail('style="border-left:none;"','',tr2);
		...
		var tr3 = complaintCaseUpdForm.createTR(table,complaintCaseUpdForm.complaintTr3+index);
		complaintCaseUpdForm.createTitle('','调度方式：',tr3);
		detail = complaintCaseUpdForm.createDetail('','',tr3);
		complaintCaseUpdForm.createSchedulingType('schedulingType'+index,detail,complaintSubObj);
		complaintCaseUpdForm.createTitle('','是否加急处理：',tr3);
		detail = complaintCaseUpdForm.createDetail('','',tr3);
		detail.attr('colspan','2');
		complaintCaseUpdForm.createIsPressDo('isPressDo'+index,detail,complaintSubObj);
		...
		btn.bind('click', function(){
			if(complaintCaseUpdForm.numberMap.size() == 1){
				alert("至少要有一个被投诉对象！");
				return;
			} 
			$('#'+complaintCaseUpdForm.complaintTr1+index).remove();
			$('#'+complaintCaseUpdForm.complaintTr2+index).remove();
			$('#'+complaintCaseUpdForm.complaintTr3+index).remove();
			complaintCaseUpdForm.numberMap.remove(index);
		});
		complaintCaseUpdForm.number = complaintCaseUpdForm.number + 1;
	},
	/**
	 * 创建TR
	 * */
	...
	createComplaintObjType:function(id,appendToCol,title,detail,index,complaintSubObj){
		...
		if(complaintCaseUpdForm.complaintObjTypeList){
			$.each(complaintCaseUpdForm.complaintObjTypeList,function(i,param){
				if(complaintSubObj!=null
					&&complaintSubObj!=""
		...
		select.bind('change',function(){
			complaintCaseUpdForm.complaintObjTypeOnChange(select.val(),title,detail,index,complaintSubObj);
		});
		select.validatebox({
			required:true
		});
	...
	complaintObjTypeOnChange:function(value,title,detail,index,complaintSubObj){
		...
		if(value == "公司部门"){
			...
			btn.bind('click',function(){
				complaintCaseUpdForm.orgDialogIndex = index;
				complaintCaseUpdForm.initOrgGrid();
		...
		if(value == "公司员工"){
			...
			btn.bind('click',function(){
				complaintCaseUpdForm.operDialogIndex = index;
				complaintCaseUpdForm.operReset();
				complaintCaseUpdForm.initOperGrid();
		...
		if(value == "服务供应商"){
			...
			btn.bind('click',function(){
				complaintCaseUpdForm.supplierDialogIndex = index;
				complaintCaseUpdForm.supplierReset();
				complaintCaseUpdForm.initSupplierGrid();
	...
	createComplaintGist:function(id,appendToCol,index){
		var select = $('<select id="'+id+'"></select>').appendTo(appendToCol);
		var defaultOption = $('<option value="">---请选择---</option>').appendTo(select);
		if(complaintCaseUpdForm.complaintGistList){
			$.each(complaintCaseUpdForm.complaintGistList,function(i,param){
				$('<option value="'+param.value+'">'+param.text+'</option>').appendTo(select);
			});
		}
		select.bind('change',function(){
	...
	createSchedulingType:function(id,appendToCol,complaintSubObj){
		var select = $('<select id="'+id+'"></select>').appendTo(appendToCol);
		var defaultOption = $('<option value="">---请选择---</option>').appendTo(select);
		if(complaintCaseUpdForm.schedulingTypeList){
			$.each(complaintCaseUpdForm.schedulingTypeList,function(i,schedulingType){
				if(complaintSubObj!=null
					&&complaintSubObj!=""
	...
	caseSave:function(handleFlag){
		...
		if(handleFlag=="SUBMIT" || handleFlag=="CLOSE"){
			if(!complaintCaseUpdForm.isValidate()){
				return;
			}
		}
		...
		$.each(complaintCaseUpdForm.numberMap.keys(),function(i,key){
			if(i==0){
		...
		$.post(url,submitForm.serialize(),function(data){
			...
			base.closeTab('complaintCaseUpdForm');
		});
	},
	/**
	 * 重置按钮事件
	...
	reset:function(){
		$("#checkForm")[0].reset();
//		$('#relationComplaint').html('');
		$('#complaintArea').html(complaintCaseUpdForm.tableHtml);
		complaintCaseUpdForm.init(complaintCaseUpdForm.caseCodeStr);
	},
	/**
	 * 页面校验
	 * */
	...
	orgDialogBtnClick:function(){
		...
		$('#complaintObjCode'+complaintCaseUpdForm.orgDialogIndex).val(node.id);
		$('#complaintObj'+complaintCaseUpdForm.orgDialogIndex).val(node.obj.orgName);
		$('#complaintObjId'+complaintCaseUpdForm.orgDialogIndex).val(node.id);
	...
	operDialogBtnClick:function(){
		...
		$('#complaintObjCode'+complaintCaseUpdForm.operDialogIndex).val(node.id);
		$('#complaintObj'+complaintCaseUpdForm.operDialogIndex).val(node.operName);
		$('#complaintObjId'+complaintCaseUpdForm.operDialogIndex).val(node.id);
	...
	seachOrgInfo:function(){
		complaintCaseUpdForm.initSubOrgGrid();
	...
	supplierDialogBtnClick:function(){
		...
		$('#complaintObjCode'+complaintCaseUpdForm.supplierDialogIndex).val(node.supName);
		$('#complaintObj'+complaintCaseUpdForm.supplierDialogIndex).val(node.supName);
		$('#complaintObjId'+complaintCaseUpdForm.supplierDialogIndex).val(node.id);
	...
	seachSupplierOrgInfo:function(){
		complaintCaseUpdForm.initSupplierSubOrgGrid();
